# Overview ----
# Southern California riparian birds case study #

# Code: Connor Wood & Mickey Pardo
# Data and fieldwork: Mickey Pardo & Connor Wood
...
## Set parameters for simulation ----
{
  ...
  # det = 0.4 for LBVI           # Secondary sampling period detection probabilities
  # det = 0.75 for BEWR           # Secondary sampling period detection probabilities
  
  visits = c(2, 4)        # Number of secondary sampling periods
  
  # occ = 0.41 for LBVI       # Starting occupancy values; for a strict specialist (occ_alt=0) total rate = occ*prop_habitat
  # occ_alt = 0  for LBVI    # occupancy rate in non-preferred habitat 
  # occ = 0.92 for BEWR       # Starting occupancy values; for a strict specialist (occ_alt=0) total rate = occ*prop_habitat
  # occ_alt = 0.5  for BEWR    # occupancy rate in non-preferred habitat 
  
  # prop_habitat = 0.37 for LBVI      # Proportion of total territories that have preferred (non-"alt") habitat
  # prop_habitat = 0.43 for BEWR      # Proportion of total territories that have preferred (non-"alt") habitat
  
  spp_type = c("lbvi")  # give qualitative names to your species types. These are NOT used in the loop
  # or "bewick's wren"
...
saveRDS(output.list, file="Round3_LBVI_1_20251022.rds")
...
outputs <- append(output.list, 
                  readRDS('D:/SoCal_Streams/Round3/Least_Bells_Vireo_2/Round3_LBVI_2_20251022.rds') )
...
write.csv(dtscen, "20260122_RMSE_and_CI_coverage_each_LBVI_scenario.csv")
